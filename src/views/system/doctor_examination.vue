<template>
  <div>
    <Card id="communist_manager">
      <h4 slot="title">
        <Select clearable style="width: 15%;" placeholder="医师定考" v-model="newsType">
    ...
    <add-doctorWork v-if="addNewsStatus" v-on:addconfirmstate="getAddNewsStatus" ></add-doctorWork>
    ...
    methods: {
      ...
      queryAllNews() {
        ...
        if (this.newsType === '全部' || this.newsType === undefined) {
          CommunistService.queryCommunistByTypeAndTitle('', 'doctorExamination', this.newsTitle).then(result => {
            if (result.code === '0') {
          ...
          CommunistService.queryCommunistByTypeAndTitle(this.newsType, 'doctorExamination', this.newsTitle).then(result => {
            if (result.code === '0') {
              this.dataTemp = result.list;
              for (let i = 0; i < this.dataTemp.length; i++) {
                this.dataTemp[i].createDate = this.fmtDate(this.dataTemp[i].createDate * 1000);
