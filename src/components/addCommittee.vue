  ...
  <div class="add-news">
    <div class="add-news-detail">
      <div class="add-ordinary-log-top">
        <div class="title">{{branchVipList !== null ? '编辑信息' : '新增信息'}}</div>
      ...
      <div class="content">
        ...
        <div class="detail">
          <div class="detail-left"><span class="span-style">*</span>分会:</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saveNews(params) {
        if (params === 0) { // 发布
          ...
          branchVipService.insertBranchVip(this.firstSelect, this.branchVip).then(result => {
            if (result.code === '0') {
              this.alertMsg('发布成功');
              this.confirm(false);
            }
    ...
    created: function() {
      this.queryNewsType(); // 分会类型
      this.branchVipList = UtilsService.getSessionStorage('committeeList');
      if (this.branchVipList !== null) { // 编辑
