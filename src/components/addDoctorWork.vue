  ...
  <div class="add-news">
    <div class="add-news-detail">
      <div class="add-ordinary-log-top" id="head">
        <div class="title">{{newsId !== '' ? '编辑医师定考' : '新增医师定考'}}</div>
        <div class="close" @click="confirm(false)"><i class="iconfont icon-style">&#xea60;</i></div>
      </div>
      <div style="clear: both;"></div>
      <div class="content">
        <div class="detail">
          <div class="detail-left"><span class="span-style">*</span>医师定考:</div>
          <div class="detail-right" style="width: 40%;text-align: left;">
            <Select id="zhifufangshi"  clearable style="width: 100%;" placeholder="请选择类型" v-model="newsInfo.newsType">
              <Option value="noticeAnnouncement">通知公告</Option>
              <Option value="policyInterpretation">政策解读</Option>
              <Option value="downloadColumn">下载专栏</Option>
            </Select>
          </div>
        </div>
        <div class="detail">
        ...
        <div class="detail">
          ...
          <div class="detail-right">
            <RadioGroup v-model="newsInfo.isCarousel" size="large">
...
//      this.$refs.myQuillEditor.quill.getModule('toolbar').addHandler('image', this.imgHandler);
    ...
    methods: {
      ...
      incheck(params) {
        ...
        if (this.newsInfo.isCarousel === '2' && this.newsInfo.titlePicture === '') {
      ...
      saveNews(params) {
        if (params === 2) { // 发布
          ...
          communistService.insertCommunist(this.newsInfo).then(result => {
            if (result.code === '0') {
              this.alertMsg('发布成功');
              this.confirm(false);
            }
...
//      },
      ...
      findNewsInfoDetailByNewsId(id) {
        communistService.findCommunistInfoDetailByNewsId(id).then(result => {
          if (result.code === '0') {
            //
            this.newsInfo.id = result.list[0].id;
            this.newsInfo.content = result.list[0].newsContent;
            this.newsInfo.newsTitle = result.list[0].title;
            this.newsInfo.newsType = result.list[0].type;
            ...
            if (result.list[0].isCarousel === null) {
              this.newsInfo.isCarousel = '1';
            }
          }
          else {
            this.alertMsg(result.msg);
    ...
    created: function() {
      let newsId = UtilsService.getSessionStorage('communistwork');// 获取communistwork中的缓存信息
      if (newsId !== null) {
