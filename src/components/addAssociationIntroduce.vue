  ...
  <div class="add-news">
    <div class="add-news-detail">
      <div class="add-ordinary-log-top" id="head">
        <div class="title">{{associationList !== null ? '编辑信息' : '新增信息'}}</div>
      ...
      <div class="content">
        <div class="detail">
          ...
          <div class="detail-right" style="width: 40%;">
            <Input  placeholder="请输入标题" style="width: 100%" v-model="associationInfo.title" :maxlength = 60 />
          </div>
        </div>
        <div class="detail">
          <div class="detail-left"><span class="span-style">*</span>类别:</div>
          <div class="detail-right" style="width: 40%;text-align: left;">
            <Select id="zhifufangshi"  clearable style="width: 50%;float: left;" placeholder="新闻类型" v-model="firstSelect">
              <Option value="associationLeader">协会领导</Option>
              <Option value="associationInfoCategory">协会信息</Option>
              <Option value="associationDepartment">职能部门</Option>
            </Select>
            <Select  clearable style="width: 50%;float: left;" placeholder="请选择分类" v-model="associationInfo.category">
        ...
        <div class="detail" style="height: 20%;">
          ...
          <div class="detail-right" style="width: 10%;height: 100%;border: 1px solid #dddee1;" @click="selectImg">
            <span v-if="!associationInfo.titlePicture">点击上传标题图片</span>
            <input class="select-Img-File-input" ref="selectImgOrFileInput" type="file" accept="image/*" @change="selectCallBack">
            <div><img v-if="associationInfo.titlePicture" :src="associationInfo.titlePicture" style="width:97%;height: 120px;"></div>
          </div>
        </div>
        <div class="detail" style="height: 370px;">
          ...
          <div class="detail-right" style="width: 90%;text-align: left;">
            <vue-ueditor-wrap v-model="associationInfo.content" :config="config"></vue-ueditor-wrap>
            <!--<quill-editor :options="editorOption" ref="myQuillEditor" v-model="associationInfo.content">-->
            <!--</quill-editor>-->
...
//  import Quill from 'quill';
  ...
  export default {
    name: 'add-AssociationIntroduce',
    props: {},
    data() {
      return {
        firstSelect: 'associationLeader',
        alertMessage: '',
        alertStates: false,
...
//      this.$refs.myQuillEditor.quill.getModule('toolbar').addHandler('image', this.imgHandler);
    ...
    methods: {
      ...
      incheck(params) {
        if (this.associationInfo.title === '') {
          this.alertMsg('标题不能为空');
          return false;
        }
        else if (this.associationInfo.category === '') {
          this.alertMsg('类型不能为空');
          return false;
        }
        else if (this.associationInfo.content === '') {
      ...
      saveNews(params) {
        this.associationInfo.type = this.firstSelect;
        if (params === 0) { // 发布
          this.associationInfo.isRelease = '2';
          associationIntroduceService.insertAssociation(this.associationInfo).then(result => {
            if (result.code === '0') {
              this.alertMsg('发布成功');
              this.confirm(false);
            }
          ...
          this.associationInfo.isRelease = '1';
          associationIntroduceService.insertAssociation(this.associationInfo).then(result => {
            if (result.code === '0') {
              this.alertMsg(result.msg);
              this.confirm(false);
      ...
      sendCallBack(val, files, imageUrl, result) {
        ...
        if (result) {
          ...
          if (val === 1) {
            ...
            this.associationInfo.titlePicture = imgUrls;
          }
          else {
    ...
    created: function() {
      this.associationList = UtilsService.getSessionStorage('associationList');
      if (this.associationList !== null) {
        this.firstSelect = this.associationList.typeCode;
        this.associationInfo.category = this.associationList.categoryCode;
        this.associationInfo.content = this.associationList.content;
        this.associationInfo.id = this.associationList.id;
        this.associationInfo.title = this.associationList.title;
        this.associationInfo.titlePicture = this.associationList.titlePicture;
      }
      this.queryNewsType();
    }
  };
